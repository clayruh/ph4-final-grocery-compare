...
class TJsScraper:
    ...
    def get_page(self):
        self.browser.get('https://www.traderjoes.com/home/products/category/fruits-116')
        time.sleep(2)
    ...
    def make_item(self):
        soup = self.get_page()
        products = soup.find_all('li', class_='ProductList_productList__item__1EIvq')

        for product in products:

            item_tag = product.find('h2', class_='ProductCard_card__title__text__uiWLe')
            img_tag = product.find('img', class_='ProductCard_card__cover__19-g3')
            price_tag = product.find('span', class_='ProductPrice_productPrice__price__3-50j')
